...
<img src="screenshot-wide.png" alt="Панель управления — широкая раскладка" width="840" />
...
1. Скачайте пакет для своей системы из [последнего релиза](https://github.com/HumSaw/yt-live-wallpaper/releases/latest):
...
- **Две темы панели** — неоновая ночь и тёплый день, с анимацией дождя в шапке
- **Автозапуск** с системой (Windows/macOS)

<div align="center">
<img src="screenshot-night.png" alt="Ночная тема" width="400" /> <img src="screenshot-day.png" alt="Дневная тема" width="400" />
</div>

## Запуск из исходников

Без терминала (Windows): дважды кликните **`run-dev.bat`** — при первом запуске он сам
...
Без терминала (Windows): дважды кликните **`build-installer.bat`** — он сам поставит зависимости,
...
```mermaid
graph TB
    subgraph renderer["Панель управления (renderer)"]
        UI["app.js — UI, темы, i18n"]
    end
    subgraph wallwin["Окно-обои"]
        VID["два слоя video + кроссфейд"]
    end
    subgraph mainproc["Main process"]
        MAIN["main.js — composition root"]
        IPC["ipc.js — обработчики IPC"]
        LIFE["app-lifecycle.js — старт/стоп, паузы"]
        WM["window-manager.js — окна, broadcast"]
        DL["downloader.js — yt-dlp, очередь"]
        FSM["fullscreen-monitor.js — детект игр"]
        WP["wallpaper.js / wallpaper-mac.js"]
        PL["playlist.js — ротация клипов"]
        RS["runtime-state.js — общие флаги"]
        ST["store.js — настройки, клипы"]
    end
    UI -- preload.js --> IPC
    IPC --> LIFE --> WM
...
- [ ] Горячие клавиши (следующий клип, пауза, звук)
- [ ] Пресеты — «полка» популярных лупов
- [ ] Поддержка Wayland
- [ ] Подписанный сертификатом установщик

Хотите что-то из списка раньше — голосуйте в
...
`~/.config/yt-live-wallpaper/videos` (Linux). Локальные файлы не копируются —
плейлист ссылается на оригинал, и при удалении клипа ваш файл остаётся на месте.
